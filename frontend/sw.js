const CACHE='tars-orbit-v11';
const SHELL=['./','./index.html','./manifest.webmanifest','./icon-192.jpg','./icon-512.jpg'];
...
self.addEventListener('fetch',event=>{
 ...
 if(isHtml){
   event.respondWith(fetch(new Request(event.request,{cache:'no-store'})).then(response=>{const copy=response.clone();caches.open(CACHE).then(c=>c.put('./index.html',copy));return response}).catch(()=>caches.match('./index.html')));
   return;
 }
